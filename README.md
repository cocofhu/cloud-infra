...
打开 **设置 → 插件 → 插件配置 → 云资源**，填写腾讯云 SecretId / SecretKey。CAM 需包含对应产品的读权限；改记录、开关机或管库还需写权限。设置页不填写地域或库账号。
...
- **CAM 未授权**：给子账号授予 DNSPod / CVM / 轻量 / CDB 相关策略后再查。单个地域未授权时其余地域仍会列出
