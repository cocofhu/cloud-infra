...
- 原始日志列表滚到底自动接着取下一页（无「继续拉取」按钮），取的过程中底部显示加载中
- 卡片顶部可按官方地域分组切换（大陆/港澳台/海外/金融/特殊）。**只改当前对话卡片，不写设置页与覆盖文件**
- 对话里查询 CDB：列表为「登录 / 管理」，管理页为官方 11 个页签；SQL 走 DMC 登录，库账号只在进程内存
- CDB / CVM / 轻量的「实例监控」为 ECharts 折线趋势图（1h / 6h / 24h 三档切换，hover 出时间戳与数值）：CDB 覆盖 CPU / 内存 / 磁盘 / 连接数 / QPS / TPS / 慢查询 / 缓存命中率 / 网络入出流量，CVM 与轻量覆盖 CPU / 内存 / 磁盘 / 内网出入带宽 / 出入包量；图表运行时从 CDN 按需加载 echarts，单指标失败只空该图
- 对话里查询镜像 / TCR / 镜像仓库时弹出**容器镜像卡片**（与域名共用 `cloud_infra_query` 通道）：顶栏先选地域（默认广州；列表走 TCR `DescribeRegions`，与控制台相同，失败时回退官方接入地域），再以卡片墙展示该地域实例；个人版实例只出现在广州
- 卡内常驻搜索框（出卡时不预填对话原句，关键字只过滤当前视图的实例/命名空间/仓库/版本），可下钻命名空间、镜像仓库与版本管理；可复制 `docker pull` 文本（不执行 docker）
- 缺凭证或企业版列表失败时在右上角弹出 CAM Message（`ToastHost`），空态写「无法加载实例」，不用「该地域没有实例」顶替；仓库/版本超过 100 条时提示仅显示前 100 条
...
- TKE：容器服务读写（集群、节点、节点池、工作负载 / 服务 / 配置 / 存储、组件、Helm、授权、策略、审计开关等对应接口）
...
4. 详情左侧按控制台分组：基本信息、节点 / 节点池、工作负载 / 服务与路由 / 配置 / 存储 / 命名空间、组件 / 应用、授权 / 策略、运维功能
...
不做：Cloud Shell、运维中心大盘、CLS 日志正文进对话。kubeconfig 只出现在集群详情 APIServer 区块，请勿让 Agent 复述。
