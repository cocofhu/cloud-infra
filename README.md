...
- 对话里查询云服务器：CVM 为顶栏中文地域 + 密表 + 行内更多；轻量为按地域分组的卡片，可切列表。实例过多时按设置里的每页条数翻页，不一次塞进对话卡
...
查询完成后对话中会显示可翻页列表（默认每页 12 条，与设置里的每页条数一致）：域名点「解析」配置记录；CVM / 轻量点实例 ID 看分组详情，或用行内更多开机 / 关机 / 重启。地域筛选只影响当次对话，不写入设置。
