...
- CDB / CVM / 轻量的「实例监控」为 ECharts 折线趋势图（1h / 6h / 24h 三档切换，hover 出时间戳与数值）：CDB 覆盖 CPU / 内存 / 磁盘 / 连接数 / QPS / TPS / 慢查询 / 缓存命中率 / 网络入出流量，CVM 与轻量覆盖 CPU / 内存 / 磁盘 / 内网出入带宽 / 磁盘读写 IOPS / 出入包量；图表运行时从 CDN 按需加载 echarts，单指标失败只空该图
