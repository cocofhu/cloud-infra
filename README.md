...
- 新建：类型卡（标准 / 弹性 / 边缘 / 注册）。标准集群四步向导（信息 → 网络 → 组件可跳 → 确认+SLA）。弹性新建入口关闭，引导标准集群 + 超级节点；独立集群不新建
- 删除向导：先关删除保护，有普通/原生/超级节点则拒绝，再选资源保留或销毁并勾选风险
- 详情侧栏：基本信息（含 Master/Node 升级、APIServer、删除保护）、节点、节点池名片、命名空间配额、组件、授权、策略、运维开关
