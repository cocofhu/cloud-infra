...
`;
    ...
    function isCloudInfraPayload(node) {
      ...
      if (Array.isArray(node.errors) && (node.kind === "cert" || node.kind === "domain" || node.kind === "auto" || node.kind === "cls" || node.kind === "dbbrain")) return true;
      return false;
    }
      ...
      return h("li", { className: "ci-cfg-item" },
        h("details", {
          ...
          h("summary", { className: "ci-cfg-h" },
            h("span", { className: "ci-cfg-t" },
              h("span", { className: "ci-cfg-n" }, "云资源"),
              h("span", { className: "ci-cfg-d" }, "配置各云厂商 AccessKey，查询域名与解析记录、DBbrain、云服务器、云数据库、对象存储与 TKE 集群。地域在资源列表中选择，不写入设置。地域只在对话卡片里选，不进设置。TKE 列表默认广州。"),
            ),
