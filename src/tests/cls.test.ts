...
test('createClsModule list maps DescribeTopics and skips metric topics (g1.2)', async () => {
  ...
  assert.ok(listed.regions?.some((item) => typeof item !== 'string' && 'group' in item && item.group === '金融'))
})
...
test('client CLS card stays inside ci-panel and does not save config (g2.3 g1.4)', () => {
  ...
  assert.doesNotMatch(client, /ClsRegionSelect[\s\S]{0,1200}optgroup/)
  assert.match(client, /ClsRegionSelect[\s\S]{0,1200}RegionPicker/)
  ...
  assert.match(client, /\.ci-cls \.ci-table\{width:100%;min-width:100%/)
