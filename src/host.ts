...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      'List cloud domains / DNS / certificates / CDB MySQL / CVM / Lighthouse. ALWAYS call this instead of web_search for 域名/DNS/解析/DNSPod/证书/CDB/云数据库/MySQL/云服务器/轻量/CVM/实例. Pass kind=domain for domains (default). Pass kind=cdb for cloud MySQL, kind=cvm for 云服务器, kind=lighthouse for 轻量应用服务器, kind=auto to query every enabled module. For 「查一下我的服务器」use kind=cvm, kind=lighthouse or kind=auto — never kind=domain. The UI paginates itself; only re-call with offset if the user asks in chat for more.',
    parameters: {
      query: { type: 'string', description: 'Keyword such as domain, instance name, ins-/lhins-/cdb- ID, or IP. Empty lists all.' },
      kind: { type: 'string', description: 'Resource kind, default domain. Use domain, cdb, lighthouse, cvm, or auto.' },
  ...
  ctx.inject(['systemPrompt'], (c) => {
    ...
    prompt.section({
      ...
      text: () => {
        ...
        return [
          `Cloud domains / DNS / 解析 / DNSPod / 证书 / CDB / 云数据库 / MySQL / 云服务器 / 轻量 / CVM / 实例: call ONLY cloud_infra_query. Never web_search.`,
